...
class CUDAMatrix(object):
    ...
    ### Suman Ravuri added functions
    def clip_min(self, min_val, target = None):
        """
        set minimum value of each element to min_val
        """
        
        err_code = clip_min(self, min_val, target)
        if err_code:
            raise generate_exception(err_code.value)
    
        return target

    def clip_max(self, max_val, target = None):
        """
        set maximum value of each element to max_val
        """
    
        return clip_max(self, max_val, target)
        if err_code:
            raise generate_exception(err_code.value)
        
        return target
            
    def clip(self, min_val, max_val, target = None):
        """
        set maximum value of each element to max_val and minimum value of each element to min_val
        """
            
        return clip(self, min_val, max_val, target)
        if err_code:
            raise generate_exception(err_code.value)
...
### Suman Ravuri added functions ####
def clip_min(mat, min_val, target = None):
    """
    sets minimum value of each element to min_val
    """

    if not target:
        target = mat

    err_code = _cudamat.clip_min(mat.p_mat, ct.c_float(min_val), target.p_mat)
    if err_code:
        raise generate_exception(err_code)

    return target

def clip_max(mat, max_val, target = None):
    """
    sets maximum value of each element to max_val
    """
    
    if not target:
        target = mat
    
    err_code = _cudamat.clip_max(mat.p_mat, ct.c_float(max_val), target.p_mat)
    if err_code:
        raise generate_exception(err_code)
    
    return target

def clip(mat, min_val, max_val, target = None):
    """
    sets minimum value of each element to min_val and maximum value of each element to max_val
    """
    
    if not target:
        target = mat

    err_code = _cudamat.clip(mat.p_mat, ct.c_float(min_val), ct.c_float(max_val), target.p_mat)
    if err_code:
        raise generate_exception(err_code)
