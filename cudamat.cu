...
extern int clip_min(cudamat* mat, float min_val, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
    ...
    kClipMin<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, min_val, target->data_device, len);
...
extern int clip_max(cudamat* mat, float max_val, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
    ...
    kClipMax<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, max_val, target->data_device, len);
...
extern int clip(cudamat* mat, float min_val, float max_val, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
    ...
    kClip<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, min_val, max_val, target->data_device, len);
